  ...
  try {
    ...
    if (response.acknowledged > 0) {
      ...
      res
        .status(500)
        .json(
          response.error || "Some error occurred while inserting the buyer"
        );
    }
  } catch (err) {
    res.status(500).json({
...
const updateBuyer = async (req, res) => {
  ...
  try {
    ...
    const response = await mongodb
      ...
      .db()
      .collection("Buyers")
      .updateOne({ _id: buyerId }, { $set: updatedBuyer });
    ...
    if (response.modifiedCount > 0) {
      ...
      res.status(500).json({
        message:
          "No changes made, check if the buyer exists or if the data is the same",
      });
    }
  } catch (err) {
    res.status(500).json({
...
const deleteBuyer = async (req, res) => {
  ...
  try {
    ...
    if (response.deletedCount > 0) {
      ...
      res
        .status(500)
        .json(response.error || "An error occurred while deleting the buyer");
    }
  } catch (err) {
    res.status(500).json({
