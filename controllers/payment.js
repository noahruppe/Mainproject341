...
const getSingle = async (req, res) => {
  ...
  try {
    ...
    const result = await mongodb
      ...
      .db()
      .collection("Payments")
      .find({ buyerId: buyerId, _id: paymentId });
    const payment = await result.toArray();
...
const createPayment = async (req, res) => {
  ...
  try {
    ...
    if (isNaN(quantity) || isNaN(price)) {
      return res
        .status(400)
        .json({ message: "Invalid quantity or price data." });
    }
...
const updatePayment = async (req, res) => {
  ...
  try {
    ...
    const response = await mongodb
      ...
      .db()
      .collection("Payments")
      .updateOne({ _id: paymentId, buyerId: buyerId }, { $set: updateFields });
    ...
    if (response.modifiedCount > 0) {
      ...
      res
        ...
        .json({
          message:
            "No payment was updated, check if the payment exists and if any values changed",
        });
    }
  } catch (err) {
    res
  ...
  try {
    ...
    if (response.deletedCount > 0) {
      ...
      res
        .status(404)
        .json({
          message: "Payment not found or not associated with this buyer.",
        });
    }
  } catch (err) {
    res
