...
const getAll = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      message: "An error occurred while fetching the seller",
      error: err,
    });
  }
};
  ...
  try {
    ...
    if (response.acknowledged > 0) {
      ...
      res
        .status(500)
        .json(
          response.error || "Some error occurred while inserting the seller"
        );
    }
  } catch (err) {
    res.status(500).json({
...
const updateSeller = async (req, res) => {
  ...
  try {
    ...
    const response = await mongodb
      ...
      .db()
      .collection("Sellers")
      .updateOne({ _id: sellerId }, { $set: updatedSeller });
...
const deleteSeller = async (req, res) => {
  ...
  try {
    ...
    if (response.deletedCount > 0) {
      ...
      res
        .status(500)
        .json(response.error || "An error occurred while deleting the seller");
    }
  } catch (err) {
    res.status(500).json({
