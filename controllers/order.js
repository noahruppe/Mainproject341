...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    if (!product) {
      return res
        .status(400)
        .json({ message: "Product not found for the specified seller." });
    }

    // Fetch buyer details by buyerId
    const buyer = await mongodb
      .getDatabase()
      .db()
      .collection("Buyers")
      .findOne({
    ...
    if (response.acknowledged) {
      res.status(201).json({
        ...
        orderId: response.insertedId, // Include the ID of the newly created order
        buyerName: order.buyerName, // Include the buyer's name
        productName: order.productName, // Include the product's name
      });
    } else {
      res
        .status(500)
...
const updateOrder = async (req, res) => {
  ...
  try {
    ...
    const response = await mongodb
      ...
      .db()
      .collection("Orders")
      .updateOne({ _id: orderId, buyerId: buyerId }, { $set: updatedFields });
    ...
    if (response.modifiedCount > 0 && req.body.quantity !== undefined) {
      ...
      if (!product) {
        return res
          .status(404)
          .json({ message: "Product not found while updating payment." });
      }
      ...
      await mongodb
        ...
        .db()
        .collection("Payments")
        .updateOne({ orderId: orderId }, { $set: { amount: totalAmount } });
    }

    res
      .status(200)
      .json({ message: "Order and payment amount updated successfully" });
  } catch (err) {
    res
      .status(500)
      .json({
...
const deleteOrder = async (req, res) => {
  ...
  try {
    ...
    if (response.deletedCount > 0) {
      res.status(204).json({ message: "Successfully deleted" }); // Successfully deleted
    } else {
      res
        .status(404)
        .json({
          message: "Order not found or not associated with this buyer.",
        });
    }
  } catch (err) {
    res
